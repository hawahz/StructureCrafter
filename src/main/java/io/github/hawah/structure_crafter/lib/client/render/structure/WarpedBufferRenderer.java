package io.github.hawah.structure_crafter.lib.client.render.structure;
...
/**
 * 缓存渲染器。
 * templateMesh是缓存的顶点数据。
 * 旧的渲染管线每次都调用原版的renderBatched，导致每次都要经过Pos -> BlockState -> BakedModel -> ... -> Vertex 的数据链。
 * 这个渲染管线通过将上述最后一步的顶点数据缓存，避免了每次渲染前都要在cpu重新处理顶点数据造成的大量卡顿。
 * 原理参考了Create的原理图渲染管线
 */
public class WarpedBufferRenderer {
    ...
    /**
     * 渲染缓存的顶点数据
     * @param poseStack: 变换矩阵，包含相机变换
     * @param builder: 顶点消费者，传入顶点之后可以在MultiBuffersource endBatch后直接渲染
     * @param level: 光照来源
     */
